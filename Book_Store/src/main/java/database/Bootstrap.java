...
public class Bootstrap {
    ...
    private static void createBookTitleUpdateTrigger() throws SQLException {
        for (String schema : SCHEMAS) {
            System.out.println("Creating trigger for " + schema);

            JDBConnectionWrapper connectionWrapper = new JDBConnectionWrapper(schema);
            Connection connection = connectionWrapper.getConnection();
            Statement statement = connection.createStatement();

            String triggerCreationSQL = new SQLTableCreationFactory().getUpdateTriggerForBookTitle();

            try {
                statement.execute(triggerCreationSQL);
