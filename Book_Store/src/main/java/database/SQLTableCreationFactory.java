...
public class SQLTableCreationFactory {
    public String getCreateSQLForTable(String table) {
        return switch (table) {
            ...
            case CART -> "CREATE TABLE IF NOT EXISTS cart (" +
                    ...
                    "  UNIQUE INDEX id_UNIQUE (id ASC)," +
                    "  INDEX user_id_idx (user_id ASC)," +
                    "  CONSTRAINT cart_user_fkid" +
                    "    FOREIGN KEY (user_id)" +
                    "    REFERENCES user (id)" +
                    "    ON DELETE CASCADE" +
                    "    ON UPDATE CASCADE," +
                    "  CONSTRAINT cart_book_fkid" +
                    "    FOREIGN KEY (book_id)" +
                    "    REFERENCES book (id)" +
                    "    ON DELETE CASCADE" +
                    "    ON UPDATE CASCADE);";
    ...
    public String getUpdateTriggerForBookTitle() {
        return "CREATE TRIGGER after_book_update_trigger " +
                "AFTER UPDATE ON book FOR EACH ROW " +
                "UPDATE cart SET book_title = NEW.title WHERE book_id = NEW.id;";
    }
}
